...
namespace DeviceHive.Client
{
    ...
    public class WebSocketChannel : Channel
    {
        ...
        private async Task OpenWebSocketAsync()
        {
            ...
            _webSocket.MessageReceived += HandleMessage;
            _webSocket.Closed += HandleConnectionClose;

            _closeTaskCompletionSource = new TaskCompletionSource<object>();

            try
            {
                await _webSocket.OpenAsync(webSocketUrl, 8010);
        ...
        private async Task<JObject> SendRequestAsync(string action, params JProperty[] args)
        {
            ...
            await _webSocket.SendAsync(requestJson.ToString());
        ...
        private void HandleMessage(IWebSocketMessage message)
        {
